...
async function createMockRepo(tempDir: string): Promise<string> {
  ...
  await ensureDir(join(repoPath, ".git"));
  await Deno.writeTextFile(join(repoPath, ".git", "HEAD"), "ref: refs/heads/main");

  return repoPath;
}

Deno.test("No Defaults Flag Behavior", async (t) => {
  const tempDir = await Deno.makeTempDir({ prefix: "claude-cleaner-no-defaults" });
  const logger = new ConsoleLogger(false);

  await t.step("should include default patterns when excludeDefaults is false", async () => {
  ...
  await t.step("should exclude default patterns when excludeDefaults is true", async () => {
    const repoPath = await createMockRepo(tempDir);
  ...
  await t.step("should handle nested Claude patterns with excludeDefaults", async () => {
    const repoPath = await createMockRepo(tempDir);

    // Create nested Claude patterns
    await ensureDir(join(repoPath, "project", ".claude"));
    await ensureDir(join(repoPath, "docs"));
    await Deno.writeTextFile(join(repoPath, "docs", "CLAUDE.md"), "# Docs Claude config");
    await ensureDir(join(repoPath, "src", ".vscode"));
    await Deno.writeTextFile(join(repoPath, "src", ".vscode", "claude.json"), "{}");
    ...
    // Should find only user pattern
    assertEquals(paths, ["project/.serena"]);
  ...
  await t.step("should handle VS Code claude.json files correctly", async () => {
    const repoPath = await createMockRepo(tempDir);
    ...
    await Deno.writeTextFile(join(repoPath, "project", ".vscode", "claude.json"), "{}");
    ...
    const fileCleanerWithDefaults = new FileCleaner(optionsWithDefaults, logger);
    ...
    const fileCleanerWithoutDefaults = new FileCleaner(optionsWithoutDefaults, logger);
  ...
  await t.step("should handle Claude temporary files correctly", async () => {
    ...
    await Deno.writeTextFile(join(repoPath, "claude-session.temp"), "session temp");
